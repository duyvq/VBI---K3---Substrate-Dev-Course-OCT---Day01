...
use frame_support::{dispatch::fmt, pallet_prelude::*};
...
pub mod pallet {
	...
	#[derive(Clone, Encode, Decode, PartialEq, Copy, Debug, TypeInfo, MaxEncodedLen)]
	pub enum Gender {
		Male,
		Female,
	}
	...
	impl<T: Config> Pallet<T> {
		#[pallet::weight(362_300_000 + T::DbWeight::get().reads_writes(5, 3))]
		pub fn create_kitty(origin: OriginFor<T>, dna: Vec<u8>) -> DispatchResult {
			...
			let _convert_moment: <<T as Config>::KittyTime as Time>::Moment = now.try_into().map_err(|_| Error::<T>::CannotConvert)?;
			...
			log::info!("New kitty is created");
			log::info!("New kitty's dna: {:?}", dna);
			log::info!("New kitty's price: {:?}", kitty.price);
			log::info!("New kitty's gender: {:?}", gender);
			log::info!("New kitty's owner: {:?}", owner);
			log::info!("New kitty's birthtime from epoch: {:?}", kitty.created_date);
		...
		#[pallet::weight(224_800_000 + T::DbWeight::get().reads_writes(3, 3))]
